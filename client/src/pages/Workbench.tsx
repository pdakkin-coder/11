...
export default function Workbench() {
  ...
  async function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (imported.warnings.length > 0) {
      toast({
        title: "Файл загружен с предупреждениями",
              ...
              <div className="p-4">
                ...
                {panel === "convert" && (
                  <ConvertPanel
                    ...
                    applyPreview={applyConversion}
                    onDiscard={() => setPreview(null)}
                    onRun={(target, converted) => setPreview({ target, text: converted })}
                  />
                )}
                {panel === "structure" && (
                  <StructurePanel structure={structure} onJump={() => {}} />
                )}
                {panel === "editor" && (
...
function DocumentView({
  text, annotations, issues, selected, hoveredId, onAnnotationClick,
}: {
  text: string;
  ...
  type Annotation = { id: string; start: number; end: number; kind: "citation" | "issue"; type: string };
  ...
  type Seg = { text: string; ann: Annotation | null; isSelected: boolean };
  ...
  const segments: Seg[] = useMemo(() => {
    ...
    for (const ann of merged) {
      ...
      if (ann.end > ann.start) {
        const isSelected = !!selected && selected.start === ann.start && selected.end === ann.end;
        segs.push({ text: text.slice(ann.start, ann.end), ann, isSelected });
      }
  ...
  const paragraphs = useMemo(() => {
    ...
    for (const seg of segments) {
      ...
      for (let i = 0; i < parts.length; i++) {
        const part = parts[i];
        if (part === "\n") {
          current.push({ text: "", ann: seg.ann, isSelected: seg.isSelected });
          result.push(current);
          current = [];
        } else if (part) {
          current.push({ text: part, ann: seg.ann, isSelected: seg.isSelected });
        }
      }
    }
  ...
  return (
    <div className="p-6 font-serif text-[14.5px] leading-[1.85] text-foreground/90 max-w-3xl mx-auto" data-testid="document-view">
      <DocumentBlocks text={text} segments={segments} selected={selected} hoveredId={hoveredId} onAnnotationClick={onAnnotationClick} />
    </div>
  );
}
...
function DocumentBlocks({ text, segments, selected, hoveredId, onAnnotationClick }: {
  text: string;
  segments: { text: string; ann: { id: string; start: number; end: number; kind: "citation" | "issue"; type: string } | null; isSelected: boolean }[];
  selected: { start: number; end: number } | null;
  hoveredId: string | null;
  onAnnotationClick: (item: FoundItem) => void;
}) {
  // Re-split segments into paragraphs
  const paragraphs: typeof segments[] = useMemo(() => {
      ...
      {paragraphs.map((para, pi) => (
        <p key={pi} className="mb-[0.6em]">
          {para.map((seg, si) => {
            ...
            const isHovered = hoveredId === seg.ann.id;
            const isSelected = seg.isSelected;
            return (
              <mark key={si}
                className={`${cls} cursor-pointer rounded-[2px] px-[1px] transition-all ${isSelected ? "ring-2 ring-primary/60" : ""} ${isHovered ? "brightness-90" : ""}`}
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-1 px-4 py-2 border-b bg-muted/20 flex-wrap">
        {[
          ...
          { icon: List,          cmd: "insertUnorderedList", title: "Список" },
      ...
      <div
        ...
        onInput={handleInput}
        spellCheck
        className="flex-1 p-6 font-serif text-[14.5px] leading-[1.85] outline-none overflow-auto"
...
function draftToHtml(plain: string): string {
  return plain
    .split("\n")
    .map((line) => `<div>${escHtml(line) || "<br>"}</div>`)
    .join("");
}
        ...
        <div className="space-y-1.5">
          {items.map((item) => (
            <button key={item.id} onClick={() => onSelect(item)} onMouseEnter={() => onHover(item.id)} onMouseLeave={() => onHover(null)}
              className="w-full text-left rounded-md border px-3 py-2 text-[12.5px] hover:bg-accent/50 transition-colors group" data-testid="citation-item">
...
function SourceTypeBuilder() {
  ...
  const preview = template ? renderSourceTemplate(template, fields) : "";
  ...
  return (
    <div className="space-y-3">
      ...
      <Select value={selectedType} onValueChange={(v) => { setSelectedType(v); setFields({}); }}>
        <SelectTrigger className="h-8 text-[12px]"><SelectValue /></SelectTrigger>
        <SelectContent>
          {DEFAULT_SOURCE_TYPES.map((t) => <SelectItem key={t.id} value={t.id}>{t.label}</SelectItem>)}
        </SelectContent>
      </Select>
      {template && (
        <div className="space-y-1.5">
          ...
          {preview && (
            <div className="rounded-md border p-2.5 bg-muted/20 mt-2">
              ...
              <div className="text-[12.5px] font-serif leading-snug">{preview}</div>
            </div>
          )}
        </div>
      )}
