...
import {
  ...
  Zap, Settings2, ChevronDown, Cpu, Eye, EyeOff,
} from "lucide-react";
...
import { buildSpans } from "@/lib/annotatedHtml";
...
export default function Workbench() {
  ...
  // ── Annotation spans for RichEditor ───────────────────────────────────────
  const spans = useMemo(
    () => buildSpans(found, issues, showIssues),
    [found, issues, showIssues],
  );
  ...
  // ── Panel selection — sync ann id ─────────────────────────────────────────
  function selectItem(item: FoundItem) {
    setSelectedAnnId(item.id);
    setSelected({ start: item.start, end: item.end });
  }

  function selectIssue(issue: EditorIssue) {
    setSelectedAnnId(issue.id);
    setSelected({ start: issue.start, end: issue.end });
  }

  // ── RichEditor callbacks ──────────────────────────────────────────────────
  const handleAnnClick = useCallback(
    (id: string) => {
      // resolve in found or issues
      const item = found.find((f) => f.id === id);
      if (item) { selectItem(item); return; }
      const iss = issues.find((i) => i.id === id);
      if (iss) { selectIssue(iss); }
    },
    [found, issues],
  );
  ...
  async function handleAiConvert() {
    ...
    const result = await runAiConvert(baseReq as Omit<AiAnalyzeRequest, "scope">, convertTargetStyle);
    if (!result) return;
    if (result.error) {
      if (result.error.includes("GEMINI_API_KEY") || result.error.includes("501")) setAiSetupOpen(true);
    ...
    if (result.convertedText?.trim()) {
      ...
      toast({
        ...
        description: `Документ переформатирован: ${convertScope.join(", ")} → ${convertTargetStyle}${modelNote}.`,
      });
    } else {
      toast({ title: "AI-конвертация завершена", description: result.summary || "Изменений не потребовалось." });
    }
  }
  ...
  async function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setDocName(imported.name); setText(imported.text); setDraft(imported.text);
    setWarnings(imported.warnings); setPreview(null); setSelected(null);
    setSelectedAnnId(null); setAiFound(null); setAiTargetStyle(null);
    if (imported.warnings.length > 0) toast({ title: "Файл загружен с предупреждениями", description: imported.warnings[0] });
  }
  ...
  async function handleUrlImport() {
    ...
    try {
      ...
      setDocName(data.name ?? "документ.txt"); setText(data.text ?? ""); setDraft(data.text ?? "");
      setWarnings(data.warnings ?? []); setPreview(null); setSelected(null);
      setSelectedAnnId(null); setLinkDialogOpen(false); setLinkUrl("");
      setAiFound(null); setAiTargetStyle(null);
      if (data.warnings?.length) toast({ title: "Импорт завершён с предупреждениями", description: data.warnings[0] });
    } catch (error) {
      toast({ title: "Не удалось загрузить ссылку", description: error instanceof Error ? error.message : "Проверьте доступность документа." });
    } finally {
  ...
  function loadDemo(id: DemoId) {
    ...
    setPreview(null); setSelected(null); setSelectedAnnId(null);
    setSearch(""); setTypeFilter("all"); setWarnings([]); setAiFound(null); setAiTargetStyle(null);
  }

  function applyConversion() {
    if (!preview) return;
    const t = preview.target;
    setText(preview.text); setDraft(preview.text); setPreview(null);
    setAiFound(null); setAiTargetStyle(null);
  ...
  // Spans for diff-preview (re-run analysis on preview text)
  const previewFound = useMemo(
    () => (preview ? findCitations(preview.text) : null),
    [preview],
  );
  const previewSpans = useMemo(
    () => (previewFound ? buildSpans(previewFound, [], false) : null),
    [previewFound],
  );
    ...
    <div className="flex flex-col h-screen bg-background text-foreground" data-testid="workbench">
      ...
      >
        <div className="flex items-center gap-3 shrink-0">
          <span className="text-primary"><CitaDexLogo size={30} /></span>
          <div className="leading-none select-none">
            <div className="text-[15px] font-bold tracking-normal text-foreground leading-tight">CitaDex</div>
            <div className="text-[11px] text-muted-foreground mt-0.5 hidden sm:block">Academic Citation Workspace</div>
          </div>
        </div>
        ...
        <div className="ml-auto flex items-center gap-2 shrink-0">
          ...
          <Button variant="outline" size="sm" onClick={handleAiAnalyze} disabled={aiLoading || !text.trim()} data-testid="button-ai-analyze" title="Анализ через Gemini AI">
          ...
          <Button
            ...
            title={showIssues ? "Скрыть замечания редактуры" : "Показать замечания редактуры"}
        ...
        <div className="h-7 border-b px-4 flex items-center gap-2 text-[11px] bg-muted/40">
          ...
          {(aiLoading || aiConvertLoading) && (
            <span className="text-muted-foreground">
              ...
              {activeRetries > 0 && (
                <span className="text-amber-600 dark:text-amber-400 ml-1">(повторная попытка {activeRetries})</span>
              )}
            </span>
          )}
          {aiStatusOk && !aiConvertLoading && !aiLoading && (
            <span className="flex items-center gap-1.5 min-w-0">
              <span>AI‑анализ активен: {aiData!.items.length} эл. — {aiData!.detectedStyle} ({Math.round((aiData!.confidence ?? 0) * 100)}%)</span>
              {aiTargetStyle && <span> · конвертация в <strong>{aiTargetStyle}</strong> применена.</span>}
              {activeModelLabel && (
          ...
          {!aiLoading && !aiConvertLoading && (aiError || aiConvertError) && (
            <button type="button" className="text-destructive underline-offset-2 hover:underline truncate" onClick={() => setAiSetupOpen(true)}>
      ...
      <div className="h-6 border-b px-4 flex items-center gap-4 text-[10px] text-muted-foreground bg-muted/20 shrink-0 overflow-x-auto">
        {(Object.entries(TYPE_LABELS) as [FoundItem["type"], string][]).map(([type, label]) => (
          <span key={type} className="flex items-center gap-1 shrink-0">
            <span className={`w-2 h-2 rounded-full ${legendDotClass(type)}`} />
            {label}
          </span>
        ))}
        {showIssues && (
          <span className="flex items-center gap-1 shrink-0">
            <span className="w-2 h-2 rounded-full legend-dot-issue" />
            Замечание редактуры
          </span>
        )}
        <span className="ml-auto flex items-center gap-1 shrink-0">
          <span className="font-medium">{found.length}</span> аннотаций
          {showIssues && issues.length > 0 && (
            <span className="ml-2"><span className="font-medium text-destructive">{issues.length}</span> замечаний</span>
          )}
        </span>
      </div>
      ...
      <Dialog open={linkDialogOpen} onOpenChange={setLinkDialogOpen}>
        <DialogContent className="sm:max-w-xl">
          ...
          <Input placeholder="https://docs.google.com/document/d/…" value={linkUrl} onChange={(e) => setLinkUrl(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleUrlImport()} />
          <DialogFooter>
      ...
      <Dialog open={aiSetupOpen} onOpenChange={setAiSetupOpen}>
        <DialogContent className="sm:max-w-lg">
          ...
          <div className="text-sm space-y-2 text-muted-foreground">
            ...
            <p className="text-[11px] pt-1">Модельный каскад: {["Gemini 2.5 Flash", "Gemini 3.5 Flash", "Gemini 3.1 Flash Lite"].join(" → ")}</p>
          </div>
      ...
      <div className="flex flex-1 min-h-0">
        ...
        {/* ── Left sidebar ──────────────────────────────────────────────────── */}
        <aside className="flex flex-col border-r bg-muted/30 shrink-0 relative" style={{ width: sidebar.width }} data-testid="sidebar">
          <nav className="flex flex-col gap-1 p-2 flex-1">
            {(Object.entries(PANEL_LABELS) as [Panel, { label: string; icon: typeof FileText }][]).map(([key, { label, icon: Icon }]) => (
              <button
                key={key} type="button" onClick={() => setPanel(key)}
          ...
          <div className="p-2 border-t">
            ...
            <Select onValueChange={(v) => loadDemo(v as DemoId)}>
              <SelectTrigger className="h-8 w-full text-xs" data-testid="select-demo"><SelectValue placeholder="Загрузить демо…" /></SelectTrigger>
              <SelectContent>{DEMO_DOCS.map((d) => <SelectItem key={d.id} value={d.id} className="text-xs">{d.label}</SelectItem>)}</SelectContent>
            </Select>
          </div>
          <div
              ...
              {panel === "structure" && (
                <div className="space-y-3">
                  ...
                  <div className="flex gap-2">
                    <Badge variant="secondary" className="text-[10px] gap-1"><Type className="h-3 w-3" />{structure.language === "ru" ? "RU" : structure.language === "en" ? "EN" : "RU+EN"}</Badge>
                    <Badge variant="secondary" className="text-[10px] gap-1"><AlignLeft className="h-3 w-3" />{structure.paragraphs} абз.</Badge>
                  </div>
                  {structure.sections.length === 0 ? (
                    ...
                    <div className="space-y-1">
                      {structure.sections.map((sec, i) => (
                        <div key={i} className="flex items-start gap-2 p-2 rounded-md hover:bg-muted/50 cursor-pointer transition-colors" style={{ paddingLeft: `${(sec.level ?? 1) * 0.5 + 0.5}rem` }}>
                  ...
                  {!aiData && <p className="text-[11px] text-muted-foreground pt-1">Запустите <span className="font-medium">AI-анализ</span> для уточнённого распознавания разделов.</p>}
                </div>
              )}
              ...
              {panel === "citations" && (
                <div className="space-y-3">
                  ...
                  <div className="relative">
                    ...
                    <Input placeholder="Поиск…" value={search} onChange={(e) => setSearch(e.target.value)} className="pl-8 h-8 text-xs" />
                  </div>
                        ...
                        >
                          <div className="flex items-center gap-1.5 mb-0.5">
                            ...
                            {item.source && item.source !== "heuristic" && (
                              <Badge variant="secondary" className="text-[9px] px-1 py-0 h-3.5 ml-0.5">{item.source === "ai" ? "AI" : "⊙"}</Badge>
                            )}
                          ...
                          <p className="text-[10px] text-muted-foreground mt-0.5">стр. {item.line}</p>
                        </div>
                      ))}
                    </div>
                  )}
                ...
                <div className="space-y-4">
                  ...
                  <div className="p-3 rounded-lg bg-muted/50 space-y-1">
                    ...
                    {aiData?.detectedStyle && aiData.detectedStyle !== detected.style && (
                      <p className="text-xs text-primary">AI уточняет: {aiData.detectedStyle} ({Math.round((aiData.confidence ?? 0) * 100)}%)</p>
                    )}
                  </div>
                  {detected.notes.length > 0 && (
                    <div className="space-y-1">
                      {detected.notes.map((n, i) => (
                        <p key={i} className="text-xs text-muted-foreground flex gap-1.5"><span className="text-primary mt-0.5">•</span>{n}</p>
                      ))}
                    </div>
                  )}
                  {detected.scores && (
                    <div className="space-y-1 pt-1">
                      <p className="text-[10px] text-muted-foreground uppercase tracking-wide">Оценки по стилям</p>
                      {detected.scores.map((s) => (
                        <div key={s.style} className="flex items-center gap-2">
                          <span className="text-xs w-20 shrink-0">{s.style}</span>
                          <div className="flex-1 h-1.5 rounded-full bg-muted overflow-hidden">
                            <div className="h-full bg-primary rounded-full" style={{ width: `${Math.round(s.score * 100)}%` }} />
                          </div>
                          <span className="text-[10px] text-muted-foreground w-8 text-right">{Math.round(s.score * 100)}%</span>
                        </div>
                      ))}
                    </div>
                  )}
